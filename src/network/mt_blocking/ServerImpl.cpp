...
void ServerImpl::Join() {
    ...
    while(!running&&socket_data.empty()){
        full_stop.wait(join_lock);
    }
}

void ServerImpl::user_Run(int client_socket) {
    ...
    std::unique_ptr<Execute::Command> command_to_execute;
        ...
        socket_data.erase(client_socket);
        close(client_socket);
        if(!running&&socket_data.empty()){
            full_stop.notify_all();
        }
}
...
void ServerImpl::OnRun() {
    ...
    while (running.load()) {
        ...
        // TODO: Start new thread and process data from/to connection
        {
            ...
            if(socket_data.size()<num_thr){
                std::thread cur_thr(&ServerImpl::user_Run, this, client_socket);
                cur_thr.detach();
                socket_data.insert(client_socket); 
            }
            else{
                close(client_socket);
            }
