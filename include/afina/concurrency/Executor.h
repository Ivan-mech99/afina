...
class Executor {
 ...
 public:
  ...
  Executor(int size, size_t min_num = 2, size_t max_num = 6,
           size_t wait_tm = 3000)
      : max_queue_size(size),
  ...
  friend void perform(Executor *executor) {
    ...
    while (executor->state == Executor::State::kRun) {
      ...
      while (executor->tasks.empty()) {
        auto result = executor->empty_condition.wait_until(
            lock, start + std::chrono::milliseconds(executor->idle_time));
    ...
    auto it = std::find_if(
        executor->threads.begin(), executor->threads.end(),
        [this_thread](std::thread &x) { return x.get_id() == this_thread; });
    it->detach();
